...
impl<G: Send + Eq + Hash + Clone> Deref for StatsReader<G> {
    type Target = evmap::ReadHandle<G, Stats>;
    fn deref(&self) -> &Self::Target {
        &self.stats
    }
}

impl<G: Send + Eq + Hash + Clone> StatsReader<G> {
    fn read(&self, game: &G) -> Option<Stats> {
        self.get_and(game, |vs| vs[0])
    }
}
...
#[derive(Clone)]
struct StatsReader<G: Send + Eq + Hash + Clone> {
